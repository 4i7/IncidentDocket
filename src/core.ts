...
function maskPatterns(): Array<[RegExp, string]> {
  const patterns: Array<[RegExp, string]> = [
    ...
    [
      /\b(?:password|passwd|pwd|token|api[_-]?key|secret|(?:[a-z0-9]+[_-])*(?:access[_-]?token|client[_-]?secret|aws[_-]access[_-]key[_-]id|aws[_-]secret[_-]access[_-]key|private[_-]?key|credentials))\b\s*[:=]\s*(?:"(?:\\.|""|[^"\\\r\n])*"|'(?:\\.|''|[^'\\\r\n])*'|"[^\r\n]*|'[^\r\n]*|[^\s,;]+)/gi,
      "<REDACTED_SECRET>",
    ],
